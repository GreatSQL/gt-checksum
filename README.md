...
- [功能新增]: 新增在 `checkObject=data` 模式下支持只校验部分字段功能，新增 `columns` 参数用于设置校验字段列表，该参数支持不同表名、字段名映射。支持同名列和源端→目标端列名映射；只比较选中的业务列，存在差异时生成 `UPDATE` 修复SQL；通过 `extraRowsSyncToSource` 参数控制是否生成 `DELETE`；当源端数据更多时生成 `columns-advisory.<schema>.<table>.sql` 文件提示人工介入处理。
- **[功能优化]** MySQL 结构修复在识别到兼容的“列重命名”场景时，改用 `CHANGE COLUMN` 代替 `DROP + ADD`；表级 `charset/collation` 修复统一显式带上 `COLLATE`，减少跨版本迁移时的默认排序规则漂移。
...
- **[测试完善]** 新增 `scripts/regression-test-columns.sh` 端到端集成回归脚本，覆盖 columns 功能 8 个核心场景（非选中列忽略、选中列修复收敛、source-only advisory、简单/完整语法、跨表列名映射、无主键 DDL-yes、target-only DELETE 修复），配套 fixture 文件 `testcase/MySQL-columns-source.sql` 和 `testcase/MySQL-columns-target.sql`；支持多轮修复收敛验证与 `--dry-run` 预览模式。
...
## Roadmap
  ...
  | 版本系列 | 目标版本 | 状态 | 方向 |
  |---------|---------|------|------|
  | v1.4.x | v1.4.0 | 开发中（需求收集中） | 计划支持 Oracle→MySQL 表结构校验与修复，以及 MySQL 前缀索引、函数索引、虚拟列等 |
  | v1.5.x | — | 规划中（需求收集中） | 方向待定 |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `columns` | 空 | 只校验部分列；支持同名列列表和 `srcSchema.srcTable.srcCol:dstSchema.dstTable.dstCol` 列映射写法 |
| `extraRowsSyncToSource` | `OFF` | 仅在 `columns` 模式下生效；`ON` 时为目标端多余行生成 `DELETE`，`OFF` 时只写 advisory 提示 |
...
其中 `resultExport`、`resultFile`、`terminalResultMode` 支持 CLI 覆盖：`--resultExport`、`--resultFile`、`--terminalResultMode`。
