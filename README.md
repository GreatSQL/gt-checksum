...
- **[功能新增]** 新增 `MariaDB -> MariaDB` 全模式支持（`data`/`struct`/`routine`/`trigger`），覆盖 `10.0` ~ `12.3` 系列，仅支持升级方向；`struct` fix 隐藏索引使用 `IGNORED` 语法，`COMPRESSED`/`PERSISTENT` 原生属性在目标端保留，`routine`/`trigger` 元数据比对同步开放；各系列特性能力按实际引入版本自动门控。
...
- **[测试完善]** 补充 MariaDB→MariaDB 版本策略、metadata gate 与列修复计划单元测试，覆盖升级链路、属性保留与元数据门控回归路径。
...
- **[问题修复]** 修复 MySQL/Oracle 修复 SQL 标识符未正确引用的问题：当索引名、schema/table 名、外键名、routine/trigger 名含空格、连字符、反引号或保留字时，生成的 SQL 无法执行；MySQL 路径统一使用 `mysqlQuoteIdent()` 转义，Oracle 路径改用正确的 `DROP INDEX` / `CREATE INDEX` 独立语句并通过 `oracleIdentifier()` 处理标识符大小写。
