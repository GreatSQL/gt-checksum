...
- 支持 `MySQL 5.6`、`5.7`、`8.0`、`8.4` 之间的同版本与升级链路校验/修复，同时新增 `MariaDB 10.x+ -> MySQL 8.0/8.4` 的 data-only 受限支持；`MySQL -> MariaDB`、`MariaDB -> MariaDB` 以及 `MariaDB -> MySQL 8.0` 以下组合会在启动阶段直接拒绝执行。
...
- 当表存在 DDL 差异时，结果总览会稳定保留该表并显示 `DDL-yes`，且 `Rows` 列固定留空；同时针对 `MariaDB` 源端移除了不准确的全局权限误报，缺失权限需以 debug 日志中的具体项为准。
...
7. **MariaDB迁移到MySQL 8.0/8.4**：在 `MariaDB 10.x+ -> MySQL 8.0/8.4` 的迁移场景中，可先执行 `checkObject=data` 的数据校验与修复，再按需要补充其他迁移验证工作。
