...
- 支持 `MySQL 5.6`、`5.7`、`8.0`、`8.4` 之间同版本及升级链路的 `data` / `struct` 校验与修复，`MySQL 8.4` 外键合法性预检查、`CHECK` 风险输出、`utf8/utf8mb3`、`ZEROFILL`、`ROW_FORMAT`、默认 `utf8mb4` 排序规则漂移等结构归一化能力已经纳入正式链路。
...
- 已提供统一的结构迁移测试基线、发布级回归基线和 fix SQL 回放标准作业说明，便于执行版本发布前的回归验证与 DBA 人工审查。
...
7. **MariaDB迁移到MySQL 8.0/8.4**：在 `MariaDB 10.x+ -> MySQL 8.0/8.4` 的迁移场景中，当前既支持 `checkObject=data` 的数据校验与修复，也支持安全子集范围内的 `checkObject=struct` 表结构校验与修复。
