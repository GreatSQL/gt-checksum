...
- 支持 `MySQL 5.6`、`5.7`、`8.0`、`8.4` 之间同版本及升级链路的 `data` / `struct` / `routine` / `trigger` 校验与修复，`MySQL 8.4` 外键合法性预检查、`CHECK` 风险输出、`utf8/utf8mb3`、`ZEROFILL`、`ROW_FORMAT`、默认 `utf8mb4` 排序规则漂移等结构归一化能力已经纳入正式链路。
- `MariaDB 10.x+ -> MySQL 8.0/8.4` 的 `checkObject` 支持已扩展到全部四种模式（`data`/`struct`/`routine`/`trigger`）；`routine` 和 `trigger` 模式新增 `CHARACTER_SET_CLIENT`、`COLLATION_CONNECTION`、`DATABASE_COLLATION` 三维度 charset 元数据比对，并支持 `MariaDB 12.3+` 的 `uca1400` 到 `uca0900` collation 映射识别。
- 支持 `MariaDB 10.5 / 10.11 -> MySQL 8.0/8.4` 的安全子集结构迁移，当前已覆盖 `JSON`、generated columns、`INET6`、`UUID`、`COMPRESSED`、`IGNORED INDEX` 等对象；`SYSTEM VERSIONING`、`WITHOUT OVERLAPS`、`SEQUENCE` 会明确输出 `warn-only` 或 advisory 信息，不会静默落成普通 `DDL mismatch`。
- 新增参数 `mariaDBJSONTargetType`，支持将 `MariaDB JSON` alias 目标类型设置为 `JSON`、`LONGTEXT` 或 `TEXT`；其中 `LONGTEXT` 路径已经完成发布级闭环验证。
- 优化存储程序定义比较策略，仅对 `CREATE PROCEDURE/FUNCTION` 头部标识符做大小写归一化，保留函数体内字符串字面量原始大小写，避免误报。
- `checkObject=data` 新增严格的 DSN `charset` 一致性校验；两端字符集不一致时会直接终止执行，避免因连接字符集不同导致数据校验失真或修复后乱码。
- `MySQL 5.6/5.7 -> 8.0/8.4` 场景新增 `INFORMATION_SCHEMA.STATISTICS.IS_VISIBLE` 兼容回退，并统一 repair SQL 前置 `session` 语句，提升跨版本数据修复兼容性。
- 当表存在 DDL 差异时，结果总览会稳定保留该表并显示 `DDL-yes`，且 `Rows` 列固定留空；结构路径中的真实残余风险会统一显示为 `warn-only`，便于区分”可审计风险”和”未收敛差异”。
- 已提供多源数据库自动化回归测试脚本（`scripts/regression-test.sh`），支持 7×2×4 测试矩阵与 `--final-repair` 完整修复验证，以及 26 个核心函数单元测试。
- 优化了列宽度收窄（Column Shrink Guard）场景的安全检查性能，由全表 `COUNT(*)` 改为 `SELECT 1 ... LIMIT 1` 短路查询，避免亿级大表上的全表扫描开销。
- 修复了当列级 COLLATION 修复 SQL 已包含 `CONVERT TO CHARACTER SET` 时，表级 collation advisory 仍重复输出的问题，fix SQL 输出更加简洁。
