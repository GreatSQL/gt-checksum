...
**MariaDB→MariaDB 支持的系列**：`10.5`、`10.6`、`10.11`、`11.4`、`11.5`、`12.3`。两端均须在此列表内且源端系列 ≤ 目标端系列；不在列表内的系列会在启动阶段直接退出。
...
2. 当源端为 `MariaDB` 时，支持 `MariaDB 10.x+ -> MySQL 8.0/8.4` 以及 `MariaDB -> MariaDB`（同系列或升级，系列范围见上方说明）两类路径；其他 `MariaDB` 组合（如 `MySQL -> MariaDB`、不支持系列、降级方向）仍会在启动阶段直接拒绝执行。
