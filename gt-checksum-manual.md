...
执行过程中若收到 `Ctrl+C` 或 `SIGTERM`，`repairDB` 会停止调度新的 SQL 文件，并等待已开始执行的文件完成后再退出；已完成文件会写入进度文件，启用续传后下次会跳过。重复发送中断信号只记录日志提示，不会强制取消正在执行的 SQL，以避免文件级续传时重放半执行文件。

**提醒**：在断点续传模式下生成的修复SQL文件可能存在重复数据冲突，会导致手动执行 `SOURCE SQL文件` 报告错误失败，需要使用 repairDB 程序执行修复工作，它会自动检测到重复数据冲突并处理，详见下方：[repairdb自动修复工具使用说明](#repairdb自动修复工具使用说明)。
...
**repairDB** 工具用于执行 SQL 修复文件，支持批量并行执行 SQL 文件并自动处理事务。内置六阶段对象类型调度模型，按对象依赖顺序分阶段执行，确保修复结果的正确性。针对并行修复过程中可能出现的死锁冲突（MySQL Error 1213），内置自动重试机制：按 `BEGIN ... COMMIT` 事务块进行重试，最多 3 次。针对 multi-values INSERT 中单行已存在导致的重复键冲突（MySQL Error 1062），会在内存中拆分为多条 single-value INSERT 逐条重试，重复行记录 `[DUPKEY-SPLIT]` 日志并跳过，其他行继续执行，且不会改写原 SQL 文件。
...
从 v2.0.1 版本开始，**repairDB** 在执行修复前会自动收集并展示修复 SQL 文件的统计信息，包括：

- 修复 SQL 文件总数和文件总大小
- 文件分类统计（DELETE / TABLE / VIEW / ROUTINE / TRIGGER / UNKNOWN）
- 数据操作统计（INSERT / UPDATE / DELETE 行数，涉及表数量）
- 结构操作统计（DROP / ALTER / CREATE 对象数）
- 预估 binlog 大小
...
从 v3.0.0 版本开始，**repairDB** 引入锁文件机制，防止重复执行修复操作：

- **锁文件位置**：`<fixFileDir>/.repairDB.lock`（隐藏文件）
- **锁文件内容**：
  - 执行成功时：空文件
  - 执行失败时：包含具体错误信息
...
**使用建议**：
1. 执行 repairDB 前，确认工作目录下不存在 `.repairDB.lock` 文件
2. 执行完成后，检查锁文件内容确认执行结果
3. 再次执行前，需手动删除锁文件：`rm <fixFileDir>/.repairDB.lock`
